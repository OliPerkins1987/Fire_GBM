...
Simplify.intention <- function(x = reported_fire$`Fire intention`, 
  ...
  y <- ifelse(x == 'Accessibility', 'Other', 
                 ...
                 ifelse(x == 'Forest clearance', 'Vegetation clearance', 
                  ifelse(x == 'Forest management', 'Crop residue burning', 
                   ifelse(x == 'Harvesting of NTFP', 'Hunter gatherer', 
                    ifelse(x == 'Harvesting of Timber', 'Vegetation clearance',
                     ...
                     ifelse(x == 'Land clearance', 'Vegetation clearance', 
                      ifelse(x == 'Pasture renewal', 'Pasture management',
                       ifelse(x == 'Pest management' & AFT %in% c('Pastoralism, Migratory', 'Cattle, Extensive', 'Cattle, Intensive'), 'Pasture management', 
                        ifelse(x == 'Pest management' & !AFT %in% c('Pastoralism, Migratory', 'Cattle, Extensive', 'Cattle, Intensive'), 'Other', 
...
DB_overview <- function(simp = T, AFT.eval = F, Abs= F) {
  ...
  if(Abs == F) {
    ...
    Simp.fire <- Simp.fire[Simp.fire$`Presence / Absence` == 'Presence', ]
    Simp.est  <- Simp.est[Simp.est$`Presence / Absence` == 'Presence', ]
  ...
  if(AFT.eval == F) {
    ...
    if(simp == T) {
      ...
      Simp.fire <- Simp.fire %>% filter(!AFT %in% c('Abandoned (arable)', 
                                                    'Agroforestry', 'Agroecologist', 'Abandoned (livestock)', 
                                                    'Cropping, Agroecology', 'Unoccupied', 'All') & !is.na(`Fire intention`))
      
      Simp.est  <- Simp.est %>% filter(!AFT %in% c('Abandoned (arable)', 
                                                   'Agroforestry', 'Agroecologist', 'Abandoned (livestock)', 
                                                   'Cropping, Agroecology', 'Unoccupied', 'All') & !is.na(`Fire intention`))
